...
basis \(X_t = S^E_t - I_t\) together with integer inventories \((q^E, q^I)\).
...
\[
\delta^{b\star}=\frac1k+X-\Delta_{q^E}^+V,
\qquad
\delta^{a\star}=\frac1k-X-\Delta_{q^E}^-V,
\qquad
\nu^\star=\frac{\partial_{q^I}V}{2\eta}.
\]
...
\[
\max\Bigl\{
  \partial_t V + \mathcal{L}^{\delta^\star,\nu^\star}V - \phi\bigl((q^E)^2+(q^I)^2\bigr),\;
  \mathcal{M}_c V - V,\;
  \mathcal{M}_r V - V
\Bigr\}=0,
\]
